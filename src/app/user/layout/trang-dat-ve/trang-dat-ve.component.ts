...
import { DanhSachPhimDangChieu } from '../../../const/data';
...
export class TrangDatVeComponent implements OnInit {
  ...
  public IDMovie: any;
  public MaNhom: any;
  // public urlHost: string = 'http://svcy.myclass.vn/Images/Movies/';
  ...
  ThongTinCheckOut() {
    ...
    if(localStorage.getItem("GheNgoiDaDat")){
      let flag = false;
      let dataGheDaDat = JSON.parse(localStorage.getItem("GheNgoiDaDat"))
      dataGheDaDat.filter(x => {
        if(parseInt(x.ShowTimeID) !== parseInt(this.IDShowTime)){
          return true
        } else {
          flag = true
          x.Seats = [...x.Seats,...ghedadat[0].Seats]
        }
      })
      !flag ? dataGheDaDat = [...dataGheDaDat,...ghedadat] : null
  ...
  GetImagePhim() {
    this.MovieDetail = DanhSachPhimDangChieu.filter(x => x.ID == this.IDMovie)[0];
